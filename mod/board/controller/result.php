        ...
        function thumbnail($arr)
        {
            ...
            for ($i = 1; $i <= sizeof($file_type); $i++) {
                if (Func::chkintd('match', $file_type[$i], SET_IMGTYPE)) {
                    ...
                    $tmb = ($fileinfo['storage'] == 'Y') ? $fileinfo['replink'] : PH_DOMAIN . MOD_BOARD_DATA_DIR2 . '/' . $board_id . '/thumb/' . $arr['file' . $i];
                }
            }
        ...
        if ($keyword) {
            ...
            switch ($req['where']) {
                ...
                case 'mb_id':
                    ...
                    break;
                default:
                    $search .= ' and (' . $where[0] . ') or (' . $where[1] . ') or (' . $where[2] . ') or (' . $where[3] . ')';
                    break;
            }
        }
